...
ArrayList<PVector> getRedPos(PImage _img){
  _img.loadPixels();
  ArrayList<PVector> reds = new ArrayList<PVector>();
  for(int i=0; i<_img.pixels.length; i++){
  float red = red(_img.pixels[i]);
  float green = green(_img.pixels[i]);
  //float blue = blue(img.pixels[i]);
  if(red>0){
    float inc = i;
    reds.add(new PVector(red, green, inc));
  }
  }
  return reds;
}
