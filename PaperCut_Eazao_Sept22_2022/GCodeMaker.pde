class GCodeMaker {
  ...
  void start(int _feedRate, float _startX, float _startY, float _startZ, int _widthTable, int _lengthTable, int _heightPrinter) {
    ...
    gCommand("; maxradius: " + maxRadius);
    gCommand("; minradius: " + minRadius);
    //gCommand("; inc: " + inc);
    ...
    gCommand("G1 Z15.0 F1500 ") ;//move the platform down 15mm
    gCommand("G92 E0 ");//zero the extruded length
    gCommand("G1 F2000 E300 ");//extrude 3mm of feed stock
  ...
  void end() {
    ...
    gCommand("G1 Z" + (endZ*1.05)); //Facilitate object removal
    //gCommand("G28 X0");
    gCommand("G90");
  }
